...
class DetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.fabFavorite.setOnClickListener {
            if (isFavorite) {
                ...
                Toast.makeText(this, "${currentEvent.name} removed from favorites", Toast.LENGTH_SHORT).show()
            } else {
                favAddViewModel.insert(FavoriteEvent(currentEvent.id.toString(), currentEvent.name, currentEvent.mediaCover))
                Toast.makeText(this, "${currentEvent.name} added to favorites", Toast.LENGTH_SHORT).show()
            }
    ...
    private fun setDetailData(event: Event) {
        ...
        binding.timeDetail.text = binding.root.context.getString(R.string.begin, formattedDate)
    ...
    private fun updateFavButton() {
        binding.fabFavorite.setImageResource(if (isFavorite) R.drawable.favorite_24 else R.drawable.favorite_border_24)
    }
